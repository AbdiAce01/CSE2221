...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        Set<String> belkin = new Set1L<>();
        boolean flag = true;

        for (String sub : strSet) { /*
                                     * check to see if strSet is a substring of
                                     * str
                                     */
        ...
        if (flag) {
            //add str to strSet and
            //remove from strSet any string that is a substring of str
            belkin.transferFrom(strSet);
            //iterate through strSet and check if any string in set is a substring of str
            while (belkin.size() > 0) {
                String ace = belkin.removeAny();
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            ...
            if (!output.contains(next)) {
                addToSetAvoidingSubstrings(output, next);
            }
        }
        return output;
