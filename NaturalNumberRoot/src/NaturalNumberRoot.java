...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        diff.copyFrom(high);
        diff.subtract(low);
        while (diff.compareTo(ONE) > 0) { /* compare high-low */
            NaturalNumber guessPower = n.newInstance();
            guessPower.copyFrom(guess);
            guessPower.power(r);

            if (n.compareTo(guessPower) < 0) { /* Interval Halving */
                high.transferFrom(guess);
            } else {
                low.transferFrom(guess);
            }

            /* Minimize Interval with each iteration */
            diff.copyFrom(high);
            diff.subtract(low);
            diff.divide(TWO);
            diff.add(low);
        ...
        n.transferFrom(low); /* Clears low and sets n equal to low */
    }
